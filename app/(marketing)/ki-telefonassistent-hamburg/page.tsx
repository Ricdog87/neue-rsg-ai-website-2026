...
export const metadata: Metadata = {
  ...
  keywords: [
    'KI Telefonassistent Hamburg',
    'AI Voice Agent Hamburg',
    'Automatischer Telefonassistent Hamburg',
    'KI Kundenservice Hamburg',
    'KI Telefonassistent Norddeutschland',
    'Voice Agent B2B Hamburg',
  ],
  alternates: { canonical: `${site.url}/ki-telefonassistent-hamburg` },
  openGraph: {
    title: 'KI-Telefonassistent Hamburg — 24/7. DSGVO. Go-Live in 2–4 Wochen.',
    description: 'KI-Telefonassistent für B2B-Unternehmen in Hamburg & Norddeutschland.',
    url: `${site.url}/ki-telefonassistent-hamburg`,
  },
  robots: { index: true, follow: true },
};

const REASONS = [
  { title: 'Hamburg schläft nicht — Ihr Agent auch nicht', body: 'Vom Hafen bis zur HafenCity: Ihr KI-Telefonassistent ist rund um die Uhr erreichbar. Internationale Anrufer, unterschiedliche Zeitzonen — kein Problem.' },
  { title: 'Für Hamburgs Branchen optimiert', body: 'Logistik, Handel, Maritime Wirtschaft, Medien & E-Commerce — der KI-Assistent spricht die Sprache Ihrer Branche und qualifiziert Leads nach Ihren Prozessen.' },
  { title: 'DSGVO + EU-Hosting · Made in Germany', body: 'Alle Sprachdaten auf deutschen Servern (Hetzner, Nürnberg). Volle DSGVO-Konformität, AVV inklusive. Für Hamburger Unternehmen mit internationalem Geschäft.' },
  { title: 'Kein Headcount, keine langen Projekte', body: 'Kein Recruiting, kein Onboarding. Ihr KI-Agent ist in 2–4 Wochen live. Skaliert sofort mit Ihrem Anrufvolumen — ohne zusätzliche Fixkosten.' },
];
const PROOF = [
...
export default function HamburgPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[hsl(var(--bg))] px-6 pb-24 pt-32 md:pt-40">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: ldJson(breadcrumbLd([{ name: 'RSG AI', url: site.url }, { name: 'KI-Telefonassistent Hamburg', url: `${site.url}/ki-telefonassistent-hamburg` }])) }} />
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <p className="mt-8 inline-flex items-center gap-2 rounded-full border border-[hsl(var(--neon))/30] bg-[hsl(var(--neon))/8] px-4 py-1.5 font-mono text-[11px] uppercase tracking-[0.22em] text-[hsl(var(--neon))]">
          <MapPin className="h-3 w-3" />Hamburg · Norddeutschland
        </p>
        <h1 className="mt-5 font-display text-balance text-4xl tracking-tight md:text-6xl lg:text-7xl">
          ...
          <span className="text-[hsl(var(--neon))]" style={{ textShadow: '0 0 40px hsl(174 100% 50% / 0.5)' }}>
            für Hamburg &amp; Norddeutschland.
          </span>
        </h1>
        <p className="mt-6 max-w-3xl text-base leading-relaxed text-[hsl(var(--muted))] md:text-lg">
          Hamburger Unternehmen setzen auf KI-Telefonie, um Wachstum zu skalieren — ohne zusätzliches Personal. Ihr Agent nimmt Anrufe entgegen, qualifiziert Leads und bucht Termine. DSGVO-konform, 24/7.
        </p>
        ...
        <section className="mt-20">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-[hsl(var(--neon))]">Warum ein KI-Telefonassistent für Hamburg?</p>
          <h2 className="mt-3 font-display text-3xl tracking-tight md:text-5xl">Vier Gründe, warum Hamburger Unternehmen auf KI-Voice setzen.</h2>
        ...
        <section className="mt-20">
          ...
          <ul className="mt-8 grid gap-3 md:grid-cols-2">
            {['Eingehende Anrufe 24/7 entgegennehmen — kein Warteton', 'Leads qualifizieren nach Ihren Kriterien (Budget, Branche, Dringlichkeit)', 'Termine direkt in Ihren Kalender buchen (Calendly, HubSpot, Google)', 'Gesprächsprotokolle & Lead-Daten automatisch ins CRM übertragen', 'Mehrsprachige Unterstützung für internationale Geschäftskontakte', 'Sprache & Ton individuell auf Ihre Marke angepasst'].map((s) => (
        ...
        <section className="mt-20 rounded-3xl border border-white/8 bg-white/[0.02] p-8 md:p-10">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-[hsl(var(--neon))]">Aktiv in Hamburg und Norddeutschland</p>
          <h2 className="mt-3 font-display text-2xl tracking-tight md:text-3xl">Von der Alster bis nach Schleswig-Holstein & Niedersachsen.</h2>
          <div className="mt-6 flex flex-wrap gap-2">
            {['Hamburg','HafenCity','Altona','Eimsbüttel','Barmbek','Wandsbek','Harburg','Norderstedt','Ahrensburg','Pinneberg','Lübeck','Bremen'].map((c) => (
        ...
        <section className="mt-20 overflow-hidden rounded-3xl border border-[hsl(var(--accent))/30] p-8 md:p-12" style={{ background: 'linear-gradient(155deg, hsl(271 91% 65% / 0.10) 0%, hsl(174 100% 50% / 0.05) 60%, hsl(240 14% 5%) 100%)', boxShadow: '0 40px 100px -30px hsl(271 91% 65% / 0.45)' }}>
          ...
          <p className="mt-5 max-w-2xl text-sm text-[hsl(var(--muted))] md:text-base">Wir zeigen Ihnen live, wie Ihr KI-Telefonassistent Anrufe annimmt, qualifiziert und Termine bucht — speziell auf Ihr Hamburger Business zugeschnitten.</p>
