...
export const metadata: Metadata = {
  title: 'KI-Telefonassistent Frankfurt | RSG AI – DSGVO-konform & 24/7',
  description:
    'KI-Telefonassistent für Unternehmen in Frankfurt am Main. Automatisch Anrufe entgegennehmen, Leads qualifizieren, Termine buchen. DSGVO-konform, Go-Live in 2–4 Wochen.',
  keywords: [
    'KI Telefonassistent Frankfurt',
    'AI Voice Agent Frankfurt',
    'Automatischer Telefonassistent Frankfurt',
    'KI Kundenservice Frankfurt',
    'KI Telefonassistent Rhein-Main',
    'Voice Agent B2B Frankfurt',
  ],
  alternates: { canonical: `${site.url}/ki-telefonassistent-frankfurt` },
  openGraph: {
    title: 'KI-Telefonassistent Frankfurt — 24/7. DSGVO. Go-Live in 2–4 Wochen.',
    description: 'KI-Telefonassistent für B2B-Unternehmen in Frankfurt & Rhein-Main.',
    url: `${site.url}/ki-telefonassistent-frankfurt`,
  },
  robots: { index: true, follow: true },
};

const REASONS = [
  { title: 'Kein Anruf bleibt unbeantwortet', body: 'Ihr KI-Telefonassistent ist 24/7 erreichbar — auch nachts, am Wochenende und während Urlaubszeiten. Kein Interessent geht mehr verloren.' },
  { title: 'Direkte CRM-Integration', body: 'Anruf-Daten fließen automatisch in HubSpot, Salesforce oder SAP. Ihre Frankfurter Vertriebsteams sehen jeden Lead sofort — ohne manuellen Aufwand.' },
  { title: 'DSGVO + EU-Hosting · Made in Germany', body: 'Sprachdaten werden in Deutschland verarbeitet (Hetzner, Nürnberg). Alle Subprozessoren unter DPF/SCC. Ihr Datenschutzbeauftragter stimmt zu.' },
  { title: 'Go-Live in 2–4 Wochen', body: 'Wir konfigurieren, testen und deployen. Ihr IT-Team wird nicht belastet. Ab Woche 2 arbeitet der Agent produktiv in Ihrer Pipeline.' },
];
const PROOF = [
...
export default function FrankfurtPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[hsl(var(--bg))] px-6 pb-24 pt-32 md:pt-40">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: ldJson(breadcrumbLd([{ name: 'RSG AI', url: site.url }, { name: 'KI-Telefonassistent Frankfurt', url: `${site.url}/ki-telefonassistent-frankfurt` }])) }} />
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <p className="mt-8 inline-flex items-center gap-2 rounded-full border border-[hsl(var(--neon))/30] bg-[hsl(var(--neon))/8] px-4 py-1.5 font-mono text-[11px] uppercase tracking-[0.22em] text-[hsl(var(--neon))]">
          <MapPin className="h-3 w-3" />Frankfurt am Main · Rhein-Main-Region
        </p>
        ...
        <p className="mt-6 max-w-3xl text-base leading-relaxed text-[hsl(var(--muted))] md:text-lg">
          Ihr KI-Telefonassistent in der Rhein-Main-Region: Nimmt Anrufe entgegen, qualifiziert Leads und bucht Termine — vollautomatisch, DSGVO-konform, rund um die Uhr.
        </p>
        ...
        <section className="mt-20">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-[hsl(var(--neon))]">Warum ein KI-Telefonassistent für Frankfurt?</p>
          <h2 className="mt-3 font-display text-3xl tracking-tight md:text-5xl">Vier Gründe, warum Frankfurter Unternehmen auf KI-Voice setzen.</h2>
        ...
        <section className="mt-20">
          ...
          <ul className="mt-8 grid gap-3 md:grid-cols-2">
            {['Eingehende Anrufe 24/7 entgegennehmen — kein Warteton', 'Leads qualifizieren nach Ihren Kriterien (Budget, Branche, Dringlichkeit)', 'Termine direkt in Ihren Kalender buchen (Calendly, HubSpot, Google)', 'Gesprächsprotokolle & Lead-Daten automatisch ins CRM übertragen', 'Anrufer auf Wunsch weiterleiten oder Rückruf-Slot vergeben', 'Sprache & Ton individuell auf Ihre Marke angepasst'].map((s) => (
        ...
        <section className="mt-20 rounded-3xl border border-white/8 bg-white/[0.02] p-8 md:p-10">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-[hsl(var(--neon))]">Aktiv in der gesamten Rhein-Main-Region</p>
          <h2 className="mt-3 font-display text-2xl tracking-tight md:text-3xl">Von Frankfurt bis ins gesamte Rhein-Main-Gebiet.</h2>
          <div className="mt-6 flex flex-wrap gap-2">
            {['Frankfurt am Main','Wiesbaden','Mainz','Offenbach','Darmstadt','Hanau','Bad Homburg','Eschborn','Rüsselsheim','Dreieich','Langen','Neu-Isenburg'].map((c) => (
        ...
        <section className="mt-20 overflow-hidden rounded-3xl border border-[hsl(var(--accent))/30] p-8 md:p-12" style={{ background: 'linear-gradient(155deg, hsl(271 91% 65% / 0.10) 0%, hsl(174 100% 50% / 0.05) 60%, hsl(240 14% 5%) 100%)', boxShadow: '0 40px 100px -30px hsl(271 91% 65% / 0.45)' }}>
          ...
          <p className="mt-5 max-w-2xl text-sm text-[hsl(var(--muted))] md:text-base">Wir zeigen Ihnen live, wie Ihr KI-Telefonassistent Anrufe annimmt, qualifiziert und Termine bucht. Kostenlos, unverbindlich.</p>
          ...
          <p className="mt-6 text-xs text-[hsl(var(--muted))]"><MapPin className="mr-1 inline h-3 w-3" />Klingholzstraße 7, 65189 Wiesbaden · Beratung auch in Frankfurt vor Ort möglich</p>
        </section>
      </div>
    </main>
  );
