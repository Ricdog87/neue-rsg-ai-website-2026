  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      if (reduce) target = isLive ? 0.42 : 0.12;
        ...
        target = 0.28 + 0.72 * burst + Math.random() * 0.05;
      } else {
        target = 0.1 + 0.05 * Math.sin(t * 1.5);
      }
      amp += (target - amp) * (reduce ? 1 : 0.18);
      ...
      const speed = reduce ? 0 : isLive ? 1.1 : 0.5;
      scan += dir * speed;
      if (scan >= SEG - 1) {
        scan = SEG - 1;
        dir = -1;
      ...
      for (let s = 0; s < SEG; s++) {
        ...
        ctx.fillStyle = rgb(0.08 + 0.85 * lvl);
      ...
      for (let i = 0; i < bars; i++) {
        ...
        const shape = Math.pow(1 - dd, 1.4);
        const n = 0.5 + 0.5 * Math.sin(t * 6 + i * 0.55) * (isLive ? 1 : 0.5);
        const h = Math.max(2, 4 + amp * eqH * 0.92 * shape * (0.55 + 0.6 * n));
...
export function VoiceConsole() {
  ...
  return (
    <div className="rounded-2xl border border-[hsl(174_100%_50%/0.25)] bg-[hsl(var(--surface))]/80 p-6 shadow-[0_0_60px_-20px_hsl(174_100%_50%/0.45)] backdrop-blur-sm">
